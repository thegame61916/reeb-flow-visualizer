...
function buildDisplayLayout(timestepValues, ranges) {
  const orderedRanges = ranges.length
    ? ranges
        .map(r => ({
          start: Math.min(+r.start, +r.end),
          end: Math.max(+r.start, +r.end)
  ...
  orderedRanges.forEach((range, rangeIndex) => {
    const values = timestepValues.filter(
    ...
    for (const node of column) {
      const h =
        totalArea > 0
          ? Math.max(
              minNodeHeight,
              available * Math.max(0, +node.area || 0) / totalArea
            )
          : fallbackHeight;
...
function renderSankey() {
  ...
  const timestepValues = [
    ...new Set(filtered.nodes.map(n => +n.timestep_index))
  ].sort((a, b) => a - b);
  ...
  const width = Math.max(
    ...
    layout.displayColumnCount * 190 + 280
  );
  ...
  const graph = d3.sankey()
    ...
    .nodeWidth(12)
    .nodePadding(18)
    .nodeAlign(d => layout.displayOrder.get(+d.timestep_index) ?? 0)
    .nodeSort(null)
    .extent([[70, 50], [width - 90, height - 50]])({
      nodes: filtered.nodes,
      links: filtered.links.map(l => ({
        ...l,
        value: Math.max(1, +l.overlap_vertices || 1)
      }))
    });
...
function updateStats(filtered) {
  ...
  const html = [
    ...
    [
      "ranges",
      controls.ranges.length
        ? controls.ranges.map(r => `${r.start}-${r.end}`).join(", ")
        : "all"
    ]
  ]
...
function showNodeDetails(d) {
  const image = d.thumbnail
    ? `<img class="thumb" src="${escapeHtml(d.thumbnail)}" alt="sheet ${d.sheet_id}">`
    : "";
...
function showLinkDetails(d) {
  ...
  const images = `
    <div class="thumb-row">
      <div>
        <h4>Source</h4>
        ${s.thumbnail ? `<img class="thumb" src="${escapeHtml(s.thumbnail)}">` : "<p>No image</p>"}
      </div>
      <div>
        <h4>Target</h4>
        ${t.thumbnail ? `<img class="thumb" src="${escapeHtml(t.thumbnail)}">` : "<p>No image</p>"}
      </div>
    </div>
  `;
...
function nodeTooltip(d) {
  ...
  return `
    <b>Sheet ${d.sheet_id}</b><br>
    time ${d.timestep_label}<br>
    rank ${d.rank}<br>
    area ${format(d.area)}<br>
    vertices ${format(d.num_vertices)}
    ${image}
  `;
}
...
function linkTooltip(d) {
  ...
  return `
    <b>${format(d.overlap_vertices)} vertices</b><br>
    ${d.source.sheet_id} → ${d.target.sheet_id}<br>
    source ${format(d.source_percent)}% target ${format(d.target_percent)}%
    <div class="tooltip-thumb-row">${sourceImage}${targetImage}</div>
  `;
}
...
function format(v) {
  return Number.isFinite(+v) ? d3.format(",.4~g")(+v) : v;
}
...
function renderRangeRows() {
  ...
  ranges.forEach((r, i) => {
    ...
    start.addEventListener("input", debounce(() => {
      ...
      if (i === selectedRangeIndex) {
        requestAnimationFrame(() => centerSelectedRange(i));
      }
    }));
...
function addRange(start = 0, end = 20) {
  ...
  selectRangeIndex(ranges.length - 1, { center: true });
}
...
function centerSelectedRange(index = selectedRangeIndex) {
  ...
  requestAnimationFrame(() => {
    chartWrap.scrollLeft = Math.max(
      0,
      (minX + maxX) / 2 - chartWrap.clientWidth / 2
    );

    chartWrap.scrollTop = Math.max(
      0,
      (minY + maxY) / 2 - chartWrap.clientHeight / 2
    );
...
function centerSankey() {
  ...
  if (zoomBehavior) {
    chart.call(zoomBehavior.transform, currentTransform);
  }
...
function startRangeDrag(event) {
  ...
  const timestep = clampTimestep(
    Math.round(minimapBarScale.invert(mx)),
    minimapBarMaxIndex
  );
...
function finishRangeDrag() {
  ...
  const finalEnd = end === start
    ? Math.min(minimapBarMaxIndex, start + 1)
    : end;
...
function visibleTimestepWindow() {
  ...
  const leftScreen = chartWrap.scrollLeft;
  const rightScreen = chartWrap.scrollLeft + chartWrap.clientWidth;

  const leftGraph = currentTransform.invertX(leftScreen);
  const rightGraph = currentTransform.invertX(rightScreen);

  return {
    start: minimapState.graphToTime(leftGraph),
    end: minimapState.graphToTime(rightGraph)
  };
}
...
function renderMiniMap() {
  ...
  rangeGroups.append("rect")
    ...
    .attr("width", d =>
      Math.max(
        8,
        x(Math.max(d.range.start, d.range.end)) -
        x(Math.min(d.range.start, d.range.end)) +
        6
      )
    )
    .attr("height", 38)
  ...
  rangeGroups.append("rect")
    .attr("class", d =>
      `range-selected${d.index === selectedRangeIndex ? " selected" : ""}`
    )
...
function resetZoomOnly() {
  ...
  const root = chart.select(".zoom-root");
  if (!root.empty()) {
    root.attr("transform", currentTransform);
  }
...
function bindControls() {
  const update = debounce(renderSankey);
  ...
  window.addEventListener("resize", debounce(renderSankey, 250));
}

d3.json("data.json").then(data => {
  fullData = data;

  const maxInitial = Math.min(
    20,
    Math.max(0, (data.timesteps || []).length - 1)
  );
